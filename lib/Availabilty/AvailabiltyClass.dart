...
class _AvailabiltyActivityState extends State<AvailabiltyActivity> {
  ...
  static Widget _eventIcon = new Container(
    decoration: new BoxDecoration(
        color: Colors.red, borderRadius: BorderRadius.all(Radius.circular(2)), border: Border.all(color: Colors.blue, width: 1.0)),
  );
  ...
  void initState() {
    ...
    selected_date = new DateFormat.d().format(today).toUpperCase() +
        ...
        " - " +
        new DateFormat.y().format(today).toUpperCase() +
        "  " +
...
*/
                  child: new Column(
                    children: [
                          ...
                          ? new InkWell(
                              onTap: () {
                                ...
                                Navigator.of(context)
                                    .push(new MaterialPageRoute(builder: (_) => new CommonDashBord("vendor_dtl", true)));
                              },
                              child: CircleAvatar(
                                radius: 55,
                      ...
                      Container(
                        alignment: Alignment.center,
                        child: new Text(
                          GlobalFile.getCaptialize(data['vendor_address']),
      ...
      apiController.GetWithMyToken(context, Url, token).then((value) {
        try {
          ...
          if (data1.length != 0) {
            ...
            setState(() {
              try {
                if (GlobalFile.getStringValue(profile['meta']['city'][0]) == "" ||
                    GlobalFile.getStringValue(profile['meta']['state'][0]) == "" ||
                    GlobalFile.getStringValue(profile['meta']['address_1'][0]) == "" ||
                    GlobalFile.getStringValue(profile['meta']['postcode'][0]) == "") {
                  Navigator.of(context)
                      .push(
                    new MaterialPageRoute(builder: (_) => new CommonDashBord("edit_bill", true)),
                  )
                      .then((val) {
  ...
  void addCartData(var index) async {
    ...
    if (await NetworkCheck.check()) {
      ...
      if (response.body.toLowerCase() == "{}") {
        ...
        Navigator.of(context).push(new MaterialPageRoute(builder: (_) => new CommonDashBord("my_cart", true)));
      }
          ...
          children: [
            ...
            Expanded(
                ...
                child: new Container(
                  ...
                  decoration: BoxDecoration(border: Border.all(color: GlobalConstant.getTextColor())),
                  child: Text(
                    AppLocalizations.of(context).translate('make_appoint'),
                    style: TextStyle(color: GlobalConstant.getTextColor(), fontWeight: FontWeight.bold, fontSize: 16.0),
                    textAlign: TextAlign.center,
                  ),
                )
                /*
  ...
  void GetSlots() async {
    ...
    String Url = GlobalConstant.CommanUrlLogin +
        "wc-appointments/v1/slots/?product_ids=${widget.data_val['id']}&min_date=$min_date&max_date=$max_date";
    ApiController apiController = new ApiController.internal();
    if (await NetworkCheck.check()) {
      Dialogs.showProgressDialog(context);
      var data1;
